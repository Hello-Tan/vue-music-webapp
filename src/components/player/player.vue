          ...
          <scroll class="middle-r" ref="lyricList" :data="currentLyric && currentLyric.lines">
            <div class="lyric-wrapper">
              <div v-if="currentLyric">
                <p class="no-lrc" v-if="!currentLyric.lines.length">{{noLrc}}</p>
                <p ref="lyricLine"
                   class="text"
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      ...
      noLrc () {
        return this.currentLyric.lrc.substr(10)
      },
      ...mapGetters([
        'fullScreen',
        'playing',
    ...
    methods: {
      ...
      fadeVolume (fade) {
        ...
        audio.volume = 1
        clearTimeout(this.timeOut)    // 防止重复点击
      ...
      getLyric () {
        this.currentSong.getLyric().then((lyric) => {
          ...
          if (!this.currentLyric.lines.length) {  // 当无歌词时，显示此内容
            this.playingLyric = this.noLrc
          }
        }).catch(() => {
    ...
    watch: {
      currentSong (newSong, oldSong) {
        ...
        if (newSong.id === oldSong.id) {   // 解决切随机播放模式自动播放问题
          return
        }
        if (this.currentLyric) {
      ...
      playing (newPlaying) {
        this.$nextTick(() => {
          newPlaying ? this.fadeVolume('fadeIn') : this.fadeVolume('fadeOut')
        })
      },
      fullScreen (val) {
        if (val) {
